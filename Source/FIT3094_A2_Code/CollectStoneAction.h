...
private:
	...
	// An array of all known tree resources. Stored so that we don't waste
	// resources doing constant collision checks. We could also have this
	// linked with world controller for easier access (A1)
	TArray<AStoneActor*> StoneResourceList;

	// Check if stone masons have gather max resources
	bool StoneResourceCollected;
