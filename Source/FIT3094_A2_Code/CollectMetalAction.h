 ...
 * 
 */
class FIT3094_A2_CODE_API CollectMetalAction : public GOAPAction
{
public:
	CollectMetalAction();
	~CollectMetalAction();
...
private:
	...
	// An array of all known mine resources. Stored so that we don't waste
	// resources doing constant collision checks. We could also have this
	// linked with world controller for easier access (A1)
	TArray<AMineActor*> MineResourceList;

	// Check if tool crafters have gather max resources
	bool MetalResourceCollected;
